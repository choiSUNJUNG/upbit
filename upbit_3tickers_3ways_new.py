...
def get_ma(ticker):
    """15일 이동 평균선 조회"""
    df = pyupbit.get_ohlcv(ticker, interval="day", count=51)
...
coin1_buy_key = 0
coin2_buy_key = 0
coin = ['KRW-POLY', 'KRW-MFT', 'KRW-META']
coin1 = ['POLY', 'MFT', 'META'] 
...
#buy
        if krw > 5000 and coin_b0 < 1 :
            ...
            df_ma = get_ma(coin[0])
            if df_ma['ma5'].iloc[-1] > df_ma['ma5'].iloc[-2] and df_ma['ma10'].iloc[-1] < current_price < (df_ma['ma5'].iloc[-1] - df_ma['ma10'].iloc[-1]) / 2 :
...
#sell 
        ...
        if coin_b1 > 1 :
            ...
            if current_price >= df_ma['upper'].iloc[-1]:    # 볼린저 상단 매도
                ...
                wb.save('upbit_deal_data.xlsx')
            elif df['low'].iloc[-2] < df_ma['lower'].iloc[-2] and current_price < df_ma['lower'].iloc[-1] and df_ma['lower'].iloc[-2] > df_ma['lower'].iloc[-1] :
                #2일 연속 볼린저 하단 발생 및 볼린저 하단가 하락시 매도
                upbit.sell_market_order(coin[1], coin_b1*0.9995)
